...
async function executeSingleTest({
  ...
  if (atsRuntime.testStopRequested) {
    console.log(
      "    🛑 Test execution stopped by user - skipping remaining tests",
    );
    return {
      testFile,
      status: "stopped",
  ...
  try {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (currentStep) {
          if (line.startsWith("msg=")) {
            ...
            currentStep.waitTime =
              parseInt(line.substring(9).replace(/["\']/g, "")) || 20;
          } else if (line.startsWith("expectedValue=")) {
            currentStep.expectedValue = line
              .substring(14)
              .replace(/["\']/g, "");
          } else if (line.startsWith("increasedBy=")) {
            currentStep.increasedBy =
              parseFloat(line.substring(12).replace(/["\']/g, "")) || 0;
        ...
        // PROPERTIES BEFORE THE STEPS
          ...
          if (line.startsWith("name=")) {
            ...
            testConfig.continueOnFail =
              parseInt(
                line.substring("continueOnFail=".length).replace(/["\']/g, ""),
                10,
              ) || 0;
          } else if (line.startsWith("retryCount=")) {
            testConfig.retryCount =
              parseInt(line.substring(11).replace(/["\']/g, "")) || 0;
      ...
      // ========== SENSOR TEST TYPE HANDLING ==========
        ...
        console.log(
          `           🔄 Running step-based test with ${testConfig.steps.length} steps`,
        );
        ...
        for (let i = 0; i < testConfig.steps.length; i++) {
          ...
          console.log(
            `    Waiting for: ${step.waitFor} = ${step.expectedValue}`,
          );
          ...
          const stepResult = await new Promise((resolve) => {
            ...
            console.log(
              "    🎯 Required Increase in current sensor test: ",
              requiredIncrease,
            );
            ...
            const cleanup = () => {
              ...
              if (currentStepHandler) {
                const idx =
                  atsRuntime.deviceCommandWaiters.indexOf(currentStepHandler);
            ...
            const stopResolver = () => {
              ...
              console.log(`🛑 Sensor step stopped by user: ${testFile}`);
            ...
            const stepHandler = (reading) => {
              ...
              console.log(
                "   📥 Current sensor value for",
                step.waitFor,
                ": ",
                reading[step.waitFor],
              );

              if (isNaN(currentValue)) {
                console.log(
                  `   ⚠️ Invalid sensor value for ${step.waitFor}: ${reading[step.waitFor]}`,
                );
                return false;
              }
              ...
              if (initialSensorValue === null) {
                ...
                console.log(
                  `   📊 Initial ${step.waitFor} value: ${initialSensorValue}`,
                );
                console.log(
                  `   🎯 Waiting for increase of: ${requiredIncrease}`,
                );
              ...
              console.log(
                `   🔍 Checking: ${step.waitFor} = ${currentValue} (initial: ${initialSensorValue}, increase: ${currentIncrease.toFixed(2)}, required: ${requiredIncrease})`,
              );
          ...
          if (stepResult.reason === "STOP_REQUESTED") {
            console.log(`🛑 ${testFile} stopped by user`);
          ...
          if (stepResult.success) {
            console.log(
              `   ✅ Step ${stepNumber} PASSED: ${step.onPass || "Success"}`,
            );
            stepResults.push({
              step: stepNumber,
              status: "passed",
            ...
            console.log(
              `   ❌ Step ${stepNumber} FAILED: ${step.onFail || stepResult.reason}`,
            );
        ...
        for (let i = 0; i < testConfig.steps.length; i++) {
          ...
          if (step.waitFor === "capture") {
            try {
              ...
              const cameraIp =
                step.cameraIp || process.env.CAMERA_IP || "192.168.0.120";
              ...
              await new Promise((resolve, reject) => {
                ...
                const stopResolver = () => {
                  ...
                  console.log(`🛑 Camera test stopped by user`);
              ...
              const dialogPromise = new Promise((resolve) => {
                ...
                const stopResolver = () => {
                  ...
                  console.log(`🛑 Camera confirmation stopped by user`);
              ...
              if (dialogResult.confirmed) {
                ...
                stepResults.push({
                  ...
                  message:
                    step.onFail ||
                    `Camera test failed - ${dialogResult.reason}`,
                  received: `Image: ${fileName}`,
                });
              ...
              console.log(
                `   Step ${stepNumber} FAILED: Camera error - ${err.message}`,
              );
        ...
        for (let i = 0; i < testConfig.steps.length; i++) {
          ...
          const stepResult = await new Promise((resolve) => {
            ...
            const stopResolver = () => {
              ...
              console.log(`🛑 ${testFile} Step ${stepNumber} stopped by user`);
            ...
            const stepHandler = (reading) => {
              ...
              if (isMultiProperty) {
                ...
                const normalizedReceived = String(receivedValue)
                  .toUpperCase()
                  .trim();
                const normalizedExpected = String(step.expectedValue)
                  .toUpperCase()
                  .trim();

                console.log(
                  `   🔍 Checking: ${step.waitFor} = "${receivedValue}" (expected: "${step.expectedValue}")`,
                );
            ...
            if (step.action) {
              ...
              for (const command of commands) {
                const finalCommand = command + getFormattedDateTime() + "$";
      ...
      // ========== EO-BASED TEST EXECUTION (Original Logic) ==========
        ...
        const multiPropertyExpectation =
          expectation.includes(":") && expectation.includes(";")
        ...
        const singlePropertyExpectation =
          expectation.includes(":") && !expectation.includes(";")
            ? expectation.split(":")
            : null;
        ...
        const waitForResponse = new Promise((resolve) => {
          const stopResolver = () => {
            ...
            console.log(`🛑 EO test stopped by user: ${testFile}`);

            clearTimeout(timeout);

            if (currentHandler) {
              const idx =
                atsRuntime.deviceCommandWaiters.indexOf(currentHandler);
          ...
          const responseHandler = (reading) => {
            ...
            if (multiPropertyExpectation) {
              console.log(
                `🔍 Multi-property check (${multiPropertyExpectation.length} properties):`,
              );
              ...
              for (const {
                ...
                console.log(
                  `⚡ATS ===  ${property}: Expected="${expectedValue}" | Received="${receivedValue}" | Match=${matches}⚡`,
                );
                results.push({
                  property,
                  expectedValue,
              ...
              if (allMatch) {
                ...
                const idx =
                  atsRuntime.deviceCommandWaiters.indexOf(responseHandler);
                if (idx > -1) atsRuntime.deviceCommandWaiters.splice(idx, 1);
                atsRuntime.clearTestWaitForMAC();
                resolve(reading);
                return true;
            ...
            if (singlePropertyExpectation) {
              ...
              console.log(
                `🔍 Property check: ${propertyName} | Expected: "${expectedValue}" | Received: "${receivedValue}" | Type: ${typeof receivedValue}`,
              );
        ...
        if (deviceResponse === "TIMEOUT") {
          ...
          if (expectation.includes(":") && expectation.includes(";")) {
            ...
            console.log(
              `📊 Multi-property comparison (${properties.length} properties):`,
            );
            ...
            for (const { property, expectedValue } of properties) {
              ...
              console.log(
                `   ${property}: Expected="${expectedValue}" | Received="${receivedValue}" | Match=${matches}`,
              );
          ...
          // Single property check (contains : but not ;)
            ...
            console.log(
              `📊 Single property comparison: ${propertyName} | Expected: ${expectedValue} | Received: ${receivedValue}`,
            );
            ...
            console.warn(
              `⚠️ EO format not recognized: "${expectation}" - expected format: "property:value" or "prop1:val1;prop2:val2"`,
            );
...
const TEST_GROUPS = {
  "full-controller": [
    // Group 1
    ["1_Fans.srv", "4_Leakage.srv", "5_Logging.srv"],
  ...
  "green-pcb": [
    // Group 1
    ["1_Fans.srv", "2_Door.srv"],

    ["4_Leakage.srv", "5_Logging.srv"],
...
const runTests = async (options) => {
  ...
  for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
    ...
    if (atsRuntime.testStopRequested) {
      console.log("🛑 Stop requested - stopping remaining test groups");
      break;
    }
    ...
    if (atsRuntime.testStopRequested) {
      ...
      console.log(`✅ TEST GROUP ${groupIndex + 1} COMPLETED`);
    }
  ...
  const passedCount = results.filter(
    (result) => result.status === "passed" || result.passed === true,
  ).length;

  const failedCount = results.filter(
    (result) => result.status === "failed" || result.passed === false,
  ).length;
