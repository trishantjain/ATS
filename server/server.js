...
// app.get("/api/readings/", async (req, res) => {
//   try {
//     const normalizedMac = req.query.mac.toLowerCase(); //! Converting to LowerCase()
//     console.log("Fetching data for MAC:", normalizedMac);
//     const latest = await SensorReading.findOne({ mac: normalizedMac })
  ...
  socket.on("data", async (data) => {
    ...
    try {
      ...
      while (buffer.length >= 58) {
        ...
        console.log("Battery Volt: ", batteryBackup);
        console.log("Fire: ", fireAlarm);

        // const failBug1 = buffer[54]; // unused
        // console.log("failBuf1: ", failBug1);

        console.log("output voltage", outputVoltage)
        console.log("Hups DVC: ", hupsDVC)
        console.log("Hups Bat Voltage: ", hupsBatVolt)
          ...
          try {
            ...
            if (cameraMake === 'H') {
              ...
              setTimeout(() => {
                ...
                const outputDir = path.join('C:', 'snaps', mac.slice(9, 17).replace(/[: ]/g, '_'));
